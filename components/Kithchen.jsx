...
const Kitchen = () => {
        ...
        const fetchOrders = async () => {
            try {
                ...
                const kitchenOrders = allOrders
                    .map((order) => {
                        const kitchenProducts = order.product.filter(
                            (product) => product.orderType === 'kitchen' && product.status !== 'close'
                        );
                        return { ...order, product: kitchenProducts };
                    })
    ...
    return (
        <div className={styles.container}>
            ...
            {orders.length === 0 ? (
                <p>No kitchen orders to display.</p>
            ) : (
